...
from utils import hexify, toLittleEndian, sha256, getLen
...
def makeRawTx(txid, vout, scriptSig, outputs):
    ...
    version = toLittleEndian(1) # version: 1
    lockTime = '00000000'
...
def makeSignedTx(output_hash, index, outputs):
    ...
    rawTx = makeRawTx(output_hash, index, scriptPubKey, outputs) # TODO: add hash code
    # sign rawTx
    txDigest = sha256(sha256(rawTx))
    signedTx = signTx(txDigest)
    # tx with scriptSig
    scriptSig = getLen(signedTx) + hexify(signedTx) + getLen(pub_key) + pub_key
